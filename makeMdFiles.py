...
# 헥소 형식으로 기록하기
def writeHexoTypeMarkdown(fileName, catName='etc'):

    originalFile = open( fileName, mode='r', encoding='utf-8')
    fixFileName = 'fix_'+catName+"_"+file_base_name(fileName)+'.md'
...
for file in fileList:
    if file.__contains__('.md'):
        ...
        if isIgnoreList(file) == False:
            writeHexoTypeMarkdown(file,catName)
